...
| 10 | Add offline command specifications | Implemented with clap-derived syntax and explicit partial validation. The 2026-09-26 audit plus subsequent annotations accounts for 159 of 169 annotated leaves; the 10 remaining paths and priorities are below. Further annotations do not change runtime contracts. Schema export and request validation remain separate. |
...
- Next order: analysis support as ordered below. Reassess lower-priority gaps
  before freezing scope; 100% semantic annotation is not a newly imposed release condition. Then qualify
...
stderr; 156 had semantic annotations at that checkpoint. Subsequent shapes and
setup-observation annotations bring coverage to 159, leaving the following 10
unannotated. The root
...
Status/ui-state now distinguish target selection and heuristic UI observations
from bar readiness.

| Priority | Exact remaining paths | Acceptance focus |
| --- | --- | --- |
| 1 | `watch compare`, `events compare`, `fundamentals` | Bounded repeated observations, event coverage and source-dependent financial interpretation. |
| 2 | `screener open`, `screener close` | Explicit UI/target effects and full-page versus dialog behavior; do not infer opening from a read request. |
| 3 | `diagnose quote-data` | Advance when diagnosing a concrete quote-data problem; state network/subscription effects and evidence limits. |
| 4 | `scanner hotlist`, `data depth`, `discover`, `spec` | Remaining discovery/self-description; depth is a heuristic DOM read, not authenticated exchange depth proof. Review value before treating annotation completion as a release gate. |
