...
groups. After chart-observation annotations, 111 paths have semantics
and 58 do not. Validation coverage remains partial even for annotated paths.
...
| --- | --- | --- |
| 1 | Remaining analysis-critical chart reads; values, strategy/trades/equity and Pine lines/labels/tables/boxes and observe chart metadata complete | Prioritize chart analysis. Explain study identity, formatted values, ambiguous strategy selection, report availability and missing data. Check downstream call sites and recorded runs before ordering supporting reads. |
