...
groups. After account-discovery annotations, 122 paths have semantics
and 47 do not. Validation coverage remains partial even for annotated paths.
...
| 3 | MCP login/status/logout; watchlist and alert list/get metadata complete | Clarify browser/credential effects, readiness and ID discovery. Advance a command if it blocks an actual chart-analysis workflow. |
