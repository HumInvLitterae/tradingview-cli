...
groups. After saved-column mutation annotations, 145 paths have semantics
and 24 do not. Validation coverage remains partial even for annotated paths.
...
| 4 | Legacy account and Desktop Screener mutations; status/get, active screen, filter/column lists and Desktop watchlist get plus screen/action/storage-column discovery and saved-screen/column mutation metadata complete | Expose account/storage/UI effects and existing MCP alternatives where equivalent. Advance earlier if a live workflow depends on them. |
