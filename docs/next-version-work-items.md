...
groups. After primary chart-stream annotations, 114 paths have semantics
and 55 do not. Validation coverage remains partial even for annotated paths.
...
| --- | --- | --- |
| 1 | Remaining analysis-critical chart reads; values, strategy/trades/equity and Pine lines/labels/tables/boxes and observe chart plus stream values/quote/bars metadata complete | Prioritize chart analysis. Explain study identity, formatted values, ambiguous strategy selection, report availability and missing data. Check downstream call sites and recorded runs before ordering supporting reads. |
