...
groups. After legacy watchlist annotations, 152 paths have semantics
and 17 do not. Validation coverage remains partial even for annotated paths.
...
| 4 | Legacy account and Desktop Screener mutations; status/get, active screen, filter/column lists and Desktop watchlist get plus screen/action/storage-column discovery and saved-screen/column/filter and legacy watchlist mutation metadata complete | Expose account/storage/UI effects and existing MCP alternatives where equivalent. Advance earlier if a live workflow depends on them. |
