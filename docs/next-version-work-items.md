...
| 10 | Add offline command specifications | Implemented: offline `tv spec` index and clap-derived command detail, with alert-history semantics and explicit partial coverage. Schema export and offline request validation follow separately. See the active work record. |
