...
`draw shape/position/list/get/remove/clear` is now implemented as a chart-local drawing lifecycle surface. `draw shape` and `draw position` return the new drawing `entity_id`, `draw get` inspects that one drawing, `draw remove` removes that exact drawing by id, and `draw clear` removes all chart-local drawings only after a `--dry-run`-capable preflight and post-clear verification. Live smoke must not use `draw clear` when pre-existing user drawings are present; create at most one test drawing and remove its returned `entity_id`.
