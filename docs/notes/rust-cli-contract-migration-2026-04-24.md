...
- `replay start`, `replay step`, `replay stop`, and `replay status` are implemented in Rust. Their payloads live under `data`; `status` preserves the practical old CLI fields `is_replay_available`, `is_replay_started`, `is_autoplay_started`, `replay_mode`, `current_date`, `autoplay_delay`, `position`, and `realized_pnl`. Basic control payloads include practical fields such as `action`, `replay_started`, `date`, `previous_date`, and `current_date`. Replay autoplay and trade commands are not implemented.
