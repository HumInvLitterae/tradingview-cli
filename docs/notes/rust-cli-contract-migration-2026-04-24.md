...
- `layout list` depends on TradingView's current page session and `window.TradingViewApi.getSavedCharts`. When that read fails or times out, Rust preserves the failure as `data.error` with an empty layout list rather than treating the read command itself as a CDP failure. `layout switch` remains deferred.
