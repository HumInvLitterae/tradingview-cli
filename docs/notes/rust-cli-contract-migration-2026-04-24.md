...
- `replay status` is implemented in Rust as a read-only command. Its payload lives under `data` and preserves the practical old CLI fields `is_replay_available`, `is_replay_started`, `is_autoplay_started`, `replay_mode`, `current_date`, `autoplay_delay`, `position`, and `realized_pnl`. Replay control commands are not implemented.
