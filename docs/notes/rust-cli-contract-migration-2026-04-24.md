...
- `data indicator`, `data strategy`, `data trades`, `data equity`, `data lines`, `data labels`, `data tables`, and `data boxes` are implemented in Rust. Their payloads live under `data`; the practical old CLI fields such as `metric_count`, `trade_count`, `data_points`, `study_count`, `studies`, `inputs`, `metrics`, `trades`, and `equity_summary` remain available.
