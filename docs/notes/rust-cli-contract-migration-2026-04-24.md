...
- `watchlist get`, `watchlist add`, `pane list`, `pane layout`, `pane focus`, and `pane symbol` are implemented in Rust. Their payloads still live under `data`; `watchlist get` may return `source: "panel_closed"` with `count: 0` when the watchlist panel is closed.
