...
- `draw shape`, `draw list`, `draw get`, `draw remove`, and `draw clear` are implemented in Rust. Their payloads live under `data`; `draw shape` exposes `action`, `shape`, `entity_id`, `new_shape_count`, `before_count`, `after_count`, `point`, `point2`, `text`, and `override_count`; `draw list` exposes `count` and `shapes`; `draw get` exposes drawing properties when available; `draw remove` exposes `action`, `entity_id`, `removed`, `before_count`, and `remaining_shapes`; `draw clear --dry-run` exposes `action: "dry_run"`, `dry_run`, `before_count`, `would_clear_count`, `cleared_entities`, and `source`; normal `draw clear` exposes `action`, `cleared`, `before_count`, `after_count`, `cleared_entities`, and `source`.
