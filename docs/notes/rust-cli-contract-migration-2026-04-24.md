...
- `screener status`, `screener open`, `screener get [--limit <N>]`, and `screener close` are implemented in Rust as read-only UI Screener dialog commands. Their payloads live under `data` and use `source: "ui_screener_dialog"`. `status` reports whether the dialog appears open, whether the toolbar button was found, and visible counts such as columns, filters, and rows. `open` and `close` may change visible UI state but do not save screens, remove filters, or change columns. `get` reads localized display text from the currently visible Screener table and returns `dialog_title`, `screen_title`, `columns`, `filters`, `rows`, `row_count`, `visible_row_count`, `limit`, `opened_for_read`, and `restored_open_state`. It rejects `--limit 0` before CDP connection, defaults to 20 rows, and caps larger limits at 100.
