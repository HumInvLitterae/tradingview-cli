...
- `tab list`, `tab switch`, `tab new`, and `tab close` are implemented in Rust. Their payloads live under `data`; `tab list` exposes old-compatible chart-target fields `tab_count` and `tabs`, plus Rust-specific `app_tab_count` and `app_tabs` for the TradingView Desktop tab strip. `tab switch` exposes `action`, `index`, `tab_id`, `chart_id`, `title`, and `url`. `tab new` exposes `action`, `source_index`, `source_tab`, `tabs_before`, `tabs_after`, `app_tabs_before`, `app_tabs_after`, `new_app_tabs`, `chart_tabs_before`, `chart_tabs_after`, and `new_tabs`. `tab close` exposes `action`, `closed_index`, `closed_tab`, `tabs_before`, `tabs_after`, `app_tabs_before`, `app_tabs_after`, `chart_tabs_before`, and `chart_tabs_after`. `tab close` is intentionally safer than the old CLI: it requires an explicit app-tab index and refuses to close the final app tab.
