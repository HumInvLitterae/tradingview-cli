...
- `layout switch <TARGET> [--dry-run]` is implemented in Rust as an explicit saved chart layout mutation. Its payload lives under `data` and includes `action`, `dry_run`, `target`, `layout`, `layout_id`, `source`, `method`, `layout_count`, `unsaved_dialog_observed`, and `unsaved_dialog_dismissed`. Rust resolves by layout id or exact case-insensitive name, not by old CLI partial matching, and it does not automatically dismiss unsaved-change dialogs.
