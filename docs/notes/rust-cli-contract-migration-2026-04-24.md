...
- `pine get`, `pine errors`, `pine console`, and `pine list` are implemented in Rust. Their payloads live under `data`; `pine get` exposes `source`, `line_count`, `char_count`, `editor_open_before`, and `opened_editor`; `pine errors` exposes `has_errors`, `error_count`, `errors`, `editor_open_before`, and `opened_editor`; `pine console` exposes `entries`, `entry_count`, `editor_open_before`, and `opened_editor`; `pine list` exposes `scripts`, `count`, `source`, and optional `error`. Pine source mutation, saving, compiling, new/open script actions, offline analyze, and server-side check remain deferred.
