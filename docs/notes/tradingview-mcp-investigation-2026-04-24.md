...
The current bridge connects to TradingView Desktop through Chrome DevTools Protocol on `localhost:9222`. The connection layer fetches `/json/list`, chooses a target whose URL looks like a TradingView chart, enables `Runtime`, `Page`, and `DOM`, then evaluates JavaScript against TradingView's Electron page. The important internal paths include `window.TradingViewApi._activeChartWidgetWV.value()`, `window.TradingViewApi._chartWidgetCollection`, and the main series bars path under `_chartWidget.model().mainSeries().bars()`.
