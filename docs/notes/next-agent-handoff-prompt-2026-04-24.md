...
Old CLI migration is closed except for MCP server implementation, which remains explicitly not planned. Narrow upstream PR follow-up is also partly complete: launch compatibility, strategy/Pine hardening, screenshot contract tests, tab target handoff, watchlist click hardening, `tv data shapes`, `tv data labels` truncation metadata, read-only `tv scanner hotlist`, read-only `tv scanner scan`, read-only `tv screener status/open/get/screens active/filters list/columns list/close`, and PR #102-style CI/agent guardrails have all been addressed. Remaining upstream-derived candidates are evidence-gated: Windows COM/AUMID launch activation only if Windows smoke proves current launch insufficient, `watchlist add-bulk` only if a real batched account-mutation workflow needs it, and layout-dialog behavior only after separate research shows core CLI value and safe operating policy. UI Screener filter/screen/column mutations remain deferred.
