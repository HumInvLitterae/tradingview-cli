...
Old CLI migration is closed except for MCP server implementation, which remains explicitly not planned. Narrow upstream PR follow-up is also partly complete: launch compatibility, strategy/Pine hardening, screenshot contract tests, tab target handoff, watchlist click hardening, API-backed `watchlist add/remove`, `tv data shapes`, `tv data labels` truncation metadata, read-only `tv scanner hotlist`, read-only `tv scanner scan`, `tv screener status/open/get/screens active/actions/list/switch/save/create/rename/save-as/delete/filters list/actions/add/modify/remove/clear/columns list/config/actions/add/remove/reorder/close` including catalog-backed screen list/switch, exact screen save, guarded test-screen create/rename/save-as/delete, evidence-gated column action discovery, storage-backed filter remove/clear, storage-backed column config/add/remove/reorder, target-popover-scoped filter action discovery, guarded add-filter catalog numeric presets, preset-backed numeric filter modify, and single-option `filters modify --option`, `tv watchlist add-bulk`, `tv quote [SYMBOL]`, PR #105 drawing smoke, and PR #102-style CI/agent guardrails have all been addressed. Remaining upstream-derived candidates are evidence-gated: alert create endpoint replacement research, Windows COM/AUMID launch activation only if Windows smoke proves current launch insufficient, layout-dialog behavior only after separate research shows core CLI value and safe operating policy, and broader multi-option/free-text filter editing only after a separate safety plan. Screener column reset was checked on 2026-04-27 and remains deferred because no post-checkable default source or visible reset action was found. Treat `filters add` and `filters modify` as guarded visible UI automation. Before adding more DOM retry logic, inspect whether the operation can use page-session storage/API evidence instead; active saved-screen storage currently supports Screener filter remove/clear by deleting existing filter entries, but not yet safe add/modify payload creation. The cross-command audit currently makes `alert create` the highest-value non-Screener replacement investigation, while `data depth`, chart screenshots, visible strategy-table fallbacks, and generic UI automation remain intentional DOM boundaries unless new evidence appears.
