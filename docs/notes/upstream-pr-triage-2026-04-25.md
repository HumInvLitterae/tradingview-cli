...
| [#66](https://github.com/tradesdontlie/tradingview-mcp/pull/66) `feat: Stock Screener tools + screen/filter/column management` | `feature` | Partially addressed. Read-oriented UI Screener dialog and metadata commands are implemented, guarded filter remove/clear commands are implemented, and Rust adds narrower menu-visible plus catalog-backed `screens list/switch` surfaces for prepared test screens. Switch commands use exact names, support dry-run target reporting, and verify the active title after mutation. Rust also implements `screens actions`, guarded `screens save`, guarded `screens create/rename/save-as`, delete dry-run target resolution, `filters actions`, guarded `filters add` for visible catalog numeric presets, and preset-backed `filters modify` for existing numeric range filters. Normal screen delete, generic non-numeric filter editing, and column mutations remain deferred. |
...
2. UI Screener and layout dialog behavior remain evidence-gated.
   ...
   `tv screener status/open/get/screens active/actions/list/switch/save/create/rename/save-as/delete/filters list/actions/add/modify/remove/clear/columns list/actions/remove --dry-run/close`, including `screens list/switch --catalog` for exact saved-screen catalog targeting. Later passes scoped filter range-option discovery to the target filter popover, added guarded filter add for visible numeric presets, added guarded screen create/rename/save-as plus delete dry-run target resolution, and made `tv tab list` expose full-page Screener CDP targets for explicit `TV_CDP_TARGET_ID` handoff. Normal screen delete, generic filter editing, and normal column mutations plus
