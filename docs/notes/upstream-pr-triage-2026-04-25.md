...
| [#66](https://github.com/tradesdontlie/tradingview-mcp/pull/66) `feat: Stock Screener tools + screen/filter/column management` | `feature` | Partially addressed. Read-oriented UI Screener dialog and metadata commands are implemented, and guarded filter remove/clear commands are implemented. Screen and column mutations remain deferred. |
...
2. UI Screener and layout dialog behavior remain evidence-gated.
   ...
   `tv screener status/open/get/screens active/filters list/remove/clear/columns list/close`. Screen and column mutations plus
