...
| [#66](https://github.com/tradesdontlie/tradingview-mcp/pull/66) `feat: Stock Screener tools + screen/filter/column management` | `feature` | Partially addressed. Read-oriented UI Screener dialog and metadata commands are implemented, guarded filter remove/clear commands are implemented, and Rust adds a narrower menu-visible `screens list/switch` surface for prepared test screens. Live smoke proved list and dry-run target reporting; non-dry-run switch currently fails safely if TradingView does not activate the clicked row. Full saved-screen catalog management, screen save/save-as/delete/rename/create, and column mutations remain deferred. |
...
2. UI Screener and layout dialog behavior remain evidence-gated.
   ...
   `tv screener status/open/get/screens active/list/switch/filters list/remove/clear/columns list/close`. Full screen catalog management, screen save/save-as/delete/rename/create, and column mutations plus
