...
self-contained and describes how to prepare the `v0.15.0` release without
adding features, refactors, dependency updates, or CI workflow changes.

## Purpose / Big Picture

The `v0.15.0` downstream evidence compatibility and quote-data diagnostics
work is complete. The pre-release audit found no blocker. This
release-readiness slice fixes the repository state for tag creation by
...
- [x] (2026-05-11T15:10Z) Audited release state, current version strings,
  README asset examples, existing release-note shape, plan index, and release
  packaging script.
- [x] (2026-05-11T15:15Z) Archived the completed v0.15 pre-release audit and
  created this release-readiness ExecPlan.
- [x] (2026-05-11T15:15Z) Bump workspace package version to `0.15.0` and
  synchronize `Cargo.lock`.
- [x] (2026-05-11T15:15Z) Cut `CHANGELOG.md` `Unreleased` entries into
  `v0.15.0 - 2026-05-12`.
- [x] (2026-05-11T15:15Z) Add `docs/releases/v0.15.0.md`.
- [x] (2026-05-11T15:15Z) Update README release asset examples and current
  roadmap / plan index.
- [x] (2026-05-11T15:16Z) Run release package validation and Rust baseline.
- [x] (2026-05-11T15:16Z) Run release safety / hygiene checks and optional
  recent CI check.
- [x] (2026-05-11T15:16Z) Prepare the release-prep changes for one local
  commit.
...
- Observation: release archive packaging still uses an explicit runtime skill
  ...
  skills and excludes development-only skills.

- Observation: `Cargo.lock` synchronized every workspace crate package entry
  to `0.15.0` after the workspace package version bump.
...
- Observation: recent remote CI status is separate from this local release
  prep.
  Evidence: `gh run list --limit 5` showed recent `v0.14.0` Release / CI and
  `main` CI runs as `completed success`, while an older `v0.13.0` Release
  workflow entry was `completed failure`. No CI workflow change was mixed into
  this release-prep slice.

## Decision Log

- Decision: Keep this slice limited to release preparation.
  Rationale: `v0.15.0` feature work and the pre-release audit are complete;
  adding fixes here would make the release-prep commit harder to review and
  tag.
  Date/Author: 2026-05-12 / Codex.

- Decision: Keep `docs/releases/v0.15.0.md` free of a top-level version
  heading.
...
`0.15.0`, `CHANGELOG.md` now contains `v0.15.0 - 2026-05-12`,
`docs/releases/v0.15.0.md` contains GitHub Release body text without a
...
returning `tv 0.15.0`, formatting, clippy, full workspace tests, metadata,
diff check, packaging script syntax check, public-doc and packaged-asset
hygiene scans, and stale asset-version scan. The optional recent CI check
showed recent `v0.14.0` and `main` runs green and an older `v0.13.0` Release
workflow failure that was not changed in this release-prep slice.
...
Acceptance is reached when package version is `0.15.0`, `Cargo.lock` is
synchronized, release notes and README asset examples reference `v0.15.0`,
