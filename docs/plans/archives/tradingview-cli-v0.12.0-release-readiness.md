...
self-contained and describes how to prepare the `v0.12.0` release without
adding features, refactors, dependency updates, or CI workflow changes.

## Purpose / Big Picture

The `v0.12.0` compare follow-up contract and missing-evidence slices are
complete, and the pre-release audit found no blocker. This release-readiness
slice fixes the repository state for tag creation by updating the Cargo package
...
- [x] (2026-05-08T09:12Z) Audited release state, current version strings,
  README asset examples, existing release-note shape, plan index, and release
  packaging script.
- [x] (2026-05-08T09:12Z) Archived the completed pre-release completion and
  refactor audit plan and created this release-readiness ExecPlan.
- [x] (2026-05-08T09:12Z) Bumped workspace package version to `0.12.0`.
- [x] (2026-05-08T09:12Z) Cut `CHANGELOG.md` `Unreleased` entries into
  `v0.12.0 - 2026-05-08`.
- [x] (2026-05-08T09:12Z) Added `docs/releases/v0.12.0.md`.
- [x] (2026-05-08T09:12Z) Updated README release asset examples and current
  roadmap / plan index.
- [x] (2026-05-08T09:12Z) Synchronized `Cargo.lock`.
- [x] (2026-05-08T09:13Z) Ran release package validation and confirmed
  `target/release/tv --version`
  prints `tv 0.12.0`.
- [x] (2026-05-08T09:16Z) Ran Rust baseline validation.
- [x] (2026-05-08T09:16Z) Ran release safety / hygiene checks and optional
  recent CI check.
- [ ] Commit the release-prep changes as one local commit.
...
- Observation: `Cargo.lock` workspace package versions synchronized to
  `0.12.0` when the workspace package version was updated.
...
- Observation: recent remote CI / release workflows are green.
  ...
  for the latest `main`, `v0.11.0`, and `v0.10.0` runs.

## Decision Log

- Decision: Keep this slice limited to release preparation.
  Rationale: `v0.12.0` feature work and pre-release audit are complete; adding
  fixes here would make the release-prep commit harder to review and tag.
  Date/Author: 2026-05-08 / Codex.

- Decision: Keep `docs/releases/v0.12.0.md` free of a top-level version
  heading.
...
`0.12.0`; `CHANGELOG.md`, README release asset examples,
`docs/releases/v0.12.0.md`, roadmap, and plans index all reflect the
...
`v0.12.0` adds additive compare follow-up contract polish. Compare payloads now
include `summary.coverage_status`, stable follow-up hint semantics, documented
field coverage semantics, requested-order join guarantees, and
`items[].missing_evidence[]`. The raw per-symbol `items[]` remains the evidence
source.
...
Acceptance is reached when package version is `0.12.0`, `Cargo.lock` is
synchronized, release notes and README asset examples reference `v0.12.0`,
...
Validation evidence:
    ...
    rg -n "v0\\.11\\.0|0\\.11\\.0" README.md docs/releases/v0.12.0.md packaging/agent/AGENTS.md CHANGELOG.md || true
    gh run list --limit 5
