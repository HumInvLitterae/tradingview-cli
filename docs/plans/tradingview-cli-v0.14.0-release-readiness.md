...
self-contained and describes how to prepare the `v0.14.0` release without
adding features, refactors, dependency updates, or CI workflow changes.

## Purpose / Big Picture

The `v0.14.0` quote-data maturity work is complete. The pre-release audit
found no blocker. This release-readiness slice fixes the repository state for
tag creation by updating the Cargo package version, changelog, GitHub Release
...
- [x] (2026-05-10T00:40Z) Audited release state, current version strings,
  README asset examples, existing release-note shape, plan index, and release
  packaging script.
- [x] (2026-05-10T00:45Z) Archived the completed v0.14 pre-release audit and
  created this release-readiness ExecPlan.
- [x] (2026-05-10T00:55Z) Bump workspace package version to `0.14.0` and synchronize
  `Cargo.lock`.
- [x] (2026-05-10T00:55Z) Cut `CHANGELOG.md` `Unreleased` entries into
  `v0.14.0 - 2026-05-10`.
- [x] (2026-05-10T00:55Z) Add `docs/releases/v0.14.0.md`.
- [x] (2026-05-10T00:55Z) Update README release asset examples and current
  roadmap / plan index.
- [x] (2026-05-10T01:10Z) Run release package validation and Rust baseline.
- [x] (2026-05-10T01:15Z) Run release safety / hygiene checks and optional
  recent CI check.
- [x] (2026-05-10T01:20Z) Prepare the release-prep changes for one local
  commit.
...
- Observation: `Cargo.lock` synchronized every workspace crate package entry
  to `0.14.0` after the workspace package version bump.
...
- Observation: recent remote CI status is separate from this local release
  prep.
  Evidence: `gh run list --limit 5` showed the latest `main` CI as
  `completed success`, while an older `v0.13.0` Release workflow entry was
  `completed failure`. No CI workflow change was mixed into this release-prep
  slice.

## Decision Log

- Decision: Keep this slice limited to release preparation.
  Rationale: `v0.14.0` feature work and the pre-release audit are complete;
  adding fixes here would make the release-prep commit harder to review and
  tag.
  Date/Author: 2026-05-10 / Codex.

- Decision: Keep `docs/releases/v0.14.0.md` free of a top-level version
  heading.
...
`0.14.0`, `CHANGELOG.md` now contains `v0.14.0 - 2026-05-10`,
`docs/releases/v0.14.0.md` contains GitHub Release body text without a
...
returning `tv 0.14.0`, formatting, clippy, full workspace tests, metadata,
diff check, packaging script syntax check, public-doc hygiene scans, and
stale asset-version scan. The optional recent CI check showed the latest
`main` CI green and an older `v0.13.0` Release workflow failure that was not
changed in this release-prep slice.
...
Acceptance is reached when package version is `0.14.0`, `Cargo.lock` is
synchronized, release notes and README asset examples reference `v0.14.0`,
