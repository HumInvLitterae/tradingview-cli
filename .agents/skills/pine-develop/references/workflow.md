...
| Open saved Pine script in TradingView | `tv pine open <NAME...>`; require verified binding |
...
Use the CLI for chart context, visual evidence, Pine read context, source validation, editor-buffer source replacement, verified saved-script open, fresh template creation, explicit save, and live editor compile verification. Write or review Pine Script in normal local files, run `tv pine analyze` for quick offline checks, run `tv pine check` for TradingView server-side compile validation, open or create the editor buffer with `tv pine open`, `tv pine new`, or `tv pine set` when useful, then run `tv pine compile` when a TradingView Desktop session is available. `tv pine open` is safe to hand to later save work only when `slot_rebound` and `binding_verified` are both true; `switch_performed: false` only means the requested script was already active and verified. A non-active script must be one unique exact row in the popup semantically linked to the Pine-owned saved-script trigger. If that row or active identity cannot be read back, do not replace the failure with `pine set` followed by save. Use `tv pine raw-compile` only when the user explicitly accepts that it preserves the old broad behavior and may click save-related Pine actions. Use `tv pine save` only when the user wants TradingView cloud persistence for the current verified saved script. Be clear that live editor compile verification may add or update a chart-local study, and that it is not a save operation. Named new-script save remains deferred because the TradingView naming dialog can be outside the CDP page target.
