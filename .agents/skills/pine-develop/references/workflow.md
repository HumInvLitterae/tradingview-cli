...
| Save Pine script in TradingView | Not implemented |
...
Use the CLI for chart context, visual evidence, Pine read context, source validation, editor-buffer source replacement, saved-script open, fresh template creation, and live editor compile verification. Write or review Pine Script in normal local files, run `tv pine analyze` for quick offline checks, run `tv pine check` for TradingView server-side compile validation, open or create the editor buffer with `tv pine open`, `tv pine new`, or `tv pine set` when useful, then run `tv pine compile` when a TradingView Desktop session is available. Be clear that live editor compile verification may add or update a chart-local study, and that it is not a save operation.

Pine raw compile and save automation remain migration backlog. The MCP server itself is not planned.
