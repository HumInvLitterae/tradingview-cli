...
The Rust `tv` CLI can now read from the Pine Editor with `tv pine get`, `tv pine errors`, and `tv pine console`, can set the current editor buffer with `tv pine set`, and can list saved scripts with `tv pine list`.

The Rust `tv pine set` command changes only the local Pine Editor buffer. The Rust `tv` CLI still does not compile Pine, save scripts in TradingView, create new Pine scripts, open saved Pine scripts, run offline Pine analysis, or run server-side Pine checks. Those old MCP workflow capabilities remain migration backlog, not completed CLI features.
...
5. Use `tv pine get`, `tv pine set`, `tv pine errors`, `tv pine console`, and `tv pine list` for Pine Editor context when TradingView Desktop is available.

## Pine Work

Write, review, or refactor Pine Script in normal project files when asked. You may use `tv pine set --file <PATH>` or stdin to place source into the editor, but do not claim the script was compiled or saved in TradingView unless that was verified by another tool or by the user.
