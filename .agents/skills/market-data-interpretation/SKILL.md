...
Replay state is the evidence under review. Read `tv replay log --steps <N>` as
bounded workflow evidence with its own Replay state and end reason, not as
OHLCV source preparation. Keep it separate from `tv bars`, scanner evidence,
quote-data, and selected-chart OHLCV.
