// Build-time provenance derivation, shared by `crates/cli/build.rs` and
// `crates/cli/tests/build_provenance.rs` through `include!`. A build script
// cannot be imported as a library, and the derivation is worth testing against
// a real repository, so both sides include this one source.
...
fn built_at(root: &Path) -> String {
    ...
    format_local_timestamp(epoch_seconds, offset_seconds)
}

/// Renders an instant as RFC 3339 in the given UTC offset.
fn format_local_timestamp(epoch_seconds: i64, offset_seconds: i64) -> String {
...
///
/// A malformed value fails the build. Falling back to the wall clock would make
/// a build that asked to be reproducible silently non-deterministic.
fn reproducible_epoch() -> Option<i64> {
    let raw = env::var("SOURCE_DATE_EPOCH").ok()?;
    match raw.trim().parse() {
        Ok(epoch_seconds) => Some(epoch_seconds),
        Err(err) => panic!("SOURCE_DATE_EPOCH is not a Unix timestamp: {raw:?} ({err})"),
    }
}

/// Local UTC offset in seconds, taken from `git` so the build timestamp and
/// commit dates use the same time-zone source. `git var` reads configuration
/// rather than a repository, so only a missing `git` forces the UTC fallback.
...
/// Paths whose change can change the stamp: executable sources for the dirty
/// marker, and the `git` state that moves on commit, checkout, or staging.
///
