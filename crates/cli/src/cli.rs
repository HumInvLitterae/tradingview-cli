...
pub enum Command {
    ...
    #[command(
        ...
        long_about = "Fetch bounded historical OHLCV bars without TradingView Desktop or CDP.\n\nThis command uses a browserless TradingView WebSocket chart-session path. SYMBOL may be exchange-qualified, such as NASDAQ:AAPL or NYSE:IONQ. A bare symbol such as AAPL is resolved through Desktop-free TradingView symbol search before the bars read; use EXCHANGE:SYMBOL when you need to force a specific exchange. Payloads report both `requested_symbol` and `resolved_symbol` so callers can see what was requested and what was actually used. By default it returns a recent count-based sample, where `--count` is capped at 500. Use `--from YYYY-MM-DD --to YYYY-MM-DD` with `--timeframe 1` (or `1m`), `5`, `15`, `30`, `60`, `1D`, `1W`, or `1M` for reproducible date-range readback; in date-range mode `--count` is a returned-bar safety cap, defaults to 500, and may be raised up to 5000, and `--to` is an inclusive calendar date. Other intraday timeframes remain guarded in date-range mode. Date-range payloads include `range_alignment` so agents can see that bar timestamps are period-start anchors and filtering uses timestamps within the requested range. Source failures may include a public-safe `source_failure_stage` that locates the symbol-search, WebSocket, setup, response, protocol, heartbeat, pagination, or result boundary; it does not authorize retry. The payload is source-labeled as `tradingview_bars_ws`, has `contract_version: \"bars.v1\"`, and does not guarantee realtime or entitlement status. `tv range` only changes the selected Desktop chart viewport, and `tv ohlcv` remains the selected-chart/CDP bars command."
    )]
    Bars {
        symbol: String,
    ...
    #[command(
        ...
        long_about = "Capture TradingView Desktop visual evidence to a local file.\n\nSupported regions are `full`, `chart`, and `strategy`. `full` captures the selected target viewport, `chart` clips to the visible chart area when detectable, and `strategy` clips to the Strategy Tester panel when it is open. Use `--wait-for-render` for an opt-in bounded stability wait before capture; `--wait-timeout-ms` controls its 500-30000 ms deadline and is only valid with the wait flag. A readiness timeout writes no file. The default remains immediate capture. This is a Desktop-backed, non-mutating read that does not change chart state, open the Strategy Tester panel, export bars, or infer strategy results."
    )]
    Screenshot {
...
pub enum ReplayCommand {
    ...
    #[command(
        ...
        long_about = "Record a bounded Replay step log as newline-delimited JSON.\n\nThis command requires an already-started TradingView Replay session, advances the selected chart by the requested number of steps, and emits readiness, step, and summary events with `contract_version: \"replay_step_log.v1\"`. Use `--attach-ohlcv-summary` to explicitly attach selected-chart OHLCV summary evidence after each successful step; `--ohlcv-count` controls that summary count and is only valid with the attachment flag. Use `--attach-chart-screenshot --screenshot-output-dir <DIR>` to write one deterministic, no-overwrite chart PNG after each successful step. Screenshot failure is reported separately and does not repeat or fail the Replay step. It does not start or stop Replay, export bars, or use Desktop-free `tv bars` as a fallback."
    )]
    Log {
        #[arg(long)]
        steps: u64,
        ...
        screenshot_output_dir: Option<std::path::PathBuf>,
    },
}
