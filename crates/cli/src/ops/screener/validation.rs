...
pub struct ScreenerFilterModifyRequest {
    ...
    pub dry_run: bool,
    pub(super) mode: ScreenerFilterModifyMode,
}

#[derive(Clone, Debug, PartialEq)]
pub(super) enum ScreenerFilterModifyMode {
    Range {
        min: Option<f64>,
        max: Option<f64>,
        preset_label: String,
...
pub struct ScreenerFilterAddRequest {
    ...
    pub max: Option<f64>,
    pub dry_run: bool,
    pub(super) range_matchers: Vec<String>,
}
...
pub(super) fn is_test_screener_screen_name(name: &str) -> bool {
    name.contains("CLI-Test") || name.contains("テスト")
}

pub(super) fn filter_modify_range_payload(request: &ScreenerFilterModifyRequest) -> Value {
    match &request.mode {
        ScreenerFilterModifyMode::Range {
            min,
            max,
...
pub(super) fn filter_modify_option_payload(option: &str, matched_option: Option<&Value>) -> Value {
    json!({
        "option": option,
...
pub(super) fn filter_add_range_payload(request: &ScreenerFilterAddRequest) -> Value {
    json!({
        "min": request.min,
        "max": request.max,
